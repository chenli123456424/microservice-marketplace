...
CREATE TABLE `custom_case` (
  ...
  `budget` DECIMAL(12,2) NOT NULL COMMENT '预算金额',
      `images` MEDIUMTEXT COMMENT '案例图片（多张图片用逗号分隔，Base64格式）',
...
CREATE TABLE `designer` (
  ...
  `avatar` TEXT COMMENT '头像图片（Base64格式）',
...
CREATE TABLE `custom_plan` (
  ...
  `icon` VARCHAR(50) COMMENT '方案图标（emoji）',
...
-- 5. 修复公告表字符集（如果存在）
-- ========================================
-- 检查announcement表是否存在，如果存在则修改字符集
...
-- 执行完成提示
-- ========================================
SELECT '全屋定制表创建完成！' AS '提示信息';
SELECT '已插入初始数据：4位设计师、3个方案、4个案例' AS '数据信息';
SELECT '如果announcement表存在，字符集已修复为utf8mb4' AS '修复信息';
